...
public class emplogin extends HttpServlet
{
		...
		throws IOException, ServletException
	{
		...
		try
		{
			...
			PreparedStatement query = dbcon.prepareStatement("SELECT fullname from employees where email=? and password=? ");
			...
			if(result.next()){
				...
				String sessionName = (String)session.getAttribute("emp");
				String check = (String)session.getAttribute("empcheck");
			    if (sessionName == null) {
			      	sessionName = employeeName;
			      	Cookie cookie = new Cookie("emp",sessionName);
					response.addCookie(cookie);
			    }
			    if(check==null){
			    	check="success";
			    }
			    session.setAttribute("emp", sessionName); 
			    session.setAttribute("empcheck", check);
			    ...
			    session.setAttribute("emp", sessionName); 
			    session.setAttribute("empcheck", check); 
				request.getRequestDispatcher("/_dashboard").include(request,response);
			}
			query.close();
			result.close();
			dbcon.close();
		...
		catch(java.lang.Exception ex)
        {
        	HttpSession session = request.getSession(true);
        	String check = (String)session.getAttribute("empcheck");
		    session.setAttribute("empcheck", "failed"); 
